...
impl<'a, C, T: Time> Queue<'a, C, T> {
    ...
    pub fn ctxt_ref(&mut self) -> &mut C {
        &mut self.ctxt
    }
    ...
    pub fn schedule_abs(&mut self, ev: impl EventCb<'a, C, T>, when: T) {
        let ev = Box::new(ev);
