...
void RdmaDriver::Init(void){
	...
	for (uint32_t i = 0; i < ipv4->GetNInterfaces (); i++){
		if (IsQbb(ipv4->GetNetDevice(i)) && ipv4->IsUp(i)){
...
void RdmaDriver::AddQueuePair(uint64_t size, bool reliable, uint16_t pg, Ipv4Address sip, Ipv4Address dip, uint16_t sport, uint16_t dport, uint32_t win, uint64_t baseRtt, Callback<void> notifyAppFinish){
	m_rdma->AddQueuePair(size, reliable, pg, sip, dip, sport, dport, win, baseRtt, notifyAppFinish);
}
